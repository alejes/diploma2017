...
class Main {
    public static void main(String[] args) {
        ...
        assert br.method_(x) == 2 // another
//int x --> Integer
        assert br.method_("one") == 0
        assert br.method_(29) == 2 //another
        assert br.method_((int) 29) == 2
        //assert br.method_(new Integer(54)) == 4
        ...
        Supplier usualSupplier = new Supplier();
        assert br.method_(usualSupplier.getInt()) == 1 // another
        assert br.method_((int) usualSupplier.getInt()) == 2
        assert br.method_(usualSupplier.getString()) == 1 // another
        assert br.method_(usualSupplier.getInteger()) == 1//another
        assert br.method_(usualSupplier.getDouble()) == 1


        println("Supplier with trait")
        TraitSupplier traitSupplier = new TraitSupplier();
        assert br.method_(traitSupplier.getInt()) == 1//another
        assert br.method_((int) traitSupplier.getInt()) == 2
        assert br.method_(traitSupplier.getString()) == 1//another
        assert br.method_(traitSupplier.getInteger()) == 1//another
...
// interesting, ambiguous for me
        ...
        assert br.method_(traitSupplier.getDerivedNull()) == 6//another
//typed null!
        assert br.method_((Derived) traitSupplier.getDerivedNull()) == 6
        assert br.method_(traitSupplier.getDerivedTypedNull()) == 6//another

        println("Ambiguous Interface Router")
        AmbiguousRouter ar = new AmbiguousRouter()
        InterfaceSupplier is = new InterfaceSupplier()
...
//^ method call ambiguous in Idea
        ...
        assert dr.method__(traitSupplier.getDerived()) == 1//another
        assert dr.method__(traitSupplier.getBase()) == 1//another
